...
- Input tokens: 65,536 / Output tokens: 32,768
- ~$0.039 per generated image
- Best for: rapid iteration, standard quality, cost-effective batch generation

### Nano Banana Pro (`gemini-3-pro-image-preview`) — High Quality
- Highest-fidelity image generation, 4K output
- Supports up to 14 reference images for style consistency
- Localized edits (modify specific regions without regenerating)
- Commercial publishing grade
- Best for: final assets, print-ready output, brand materials

Use the `generate` task for Nano Banana (default) or `generate-hq` for Nano Banana Pro.
...
python scripts/gemini_batch_process.py \
  ...
  --output docs/assets/landscape.png

# High-quality generation (Nano Banana Pro)
python scripts/gemini_batch_process.py \
  --task generate-hq \
  --prompt "A serene mountain landscape at sunset, 4K detail" \
  --output docs/assets/landscape-hq.png \
