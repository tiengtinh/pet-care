...
**`gemini-3-flash-preview`** — fast, cost-effective multimodal processing for audio tasks. For budget-constrained batch jobs, `gemini-2.5-flash` remains a stable alternative.
...
> **Note:** The `gemini-2.5-flash-native-audio-preview-09-2025` model ID is a dated preview identifier and may have been superseded. Check the [models page](https://ai.google.dev/gemini-api/docs/models) for the current TTS model ID.
...
**Model selection**:
- Use `gemini-3-flash-preview` for most tasks (best quality/cost)
- Use `gemini-2.5-flash` for budget-constrained or production-stable workloads
- Upgrade to `gemini-3.1-pro-preview` for complex analysis
